...
ifeq ($(KERNEL_DEFCONFIG),)
   ifeq ($(TARGET_BUILD_VARIANT),user)
     KERNEL_DEFCONFIG := vendor/$(TARGET_PRODUCT)-perf_defconfig
   else
     KERNEL_DEFCONFIG := vendor/$(TARGET_PRODUCT)_defconfig
   endif
endif
...
KERNEL_SRC_DEP := $(shell find $(SOURCE_ROOT)/$(TARGET_KERNEL_SOURCE)/ -type f -name "*")
KERNEL_SRC_DEP += $(foreach item, $(shell find $(SOURCE_ROOT)/$(TARGET_KERNEL_SOURCE)/ -type l -not -path "*/.git/*"|xargs realpath|grep -v "kernel\/$(TARGET_KERNEL)"), $(shell find ${item} -name "*" -type f -not -path "*/.git/*" 2>/dev/null))
...
#ifdef COLOR_EDIT
my_feature_file := $(SOURCE_ROOT)/kernel/oplus_native_features.sh
...
#	$(TARGET_KERNEL_MAKE_ENV)
#else OPLUS_FEATURE_FORCE_SELINUX
$(TARGET_PREBUILT_KERNEL): $(DTC) $(KERNEL_USR) $(KERNEL_SRC_DEP) | $(KERNEL_OUT)
	KERNEL_DIR=$(TARGET_KERNEL_SOURCE) \
	DEFCONFIG=$(KERNEL_DEFCONFIG) \
	OUT_DIR=$(KERNEL_OUT) \
	MAKE_PATH=$(MAKE_PATH) \
