...
bool dsi_panel_need_rewrite_reg = false;
...
int store_index = 0;
bool dsi_panel_need_reset_count = false;
...
char oplus_post_on_backlight_status = 0x0;
		...
		void *display, u32 bl_lvl)
{
	...
	if (panel->need_power_on_backlight && bl_lvl) {
		...
		oplus_post_on_backlight_status = 0x1;
		rc = dsi_display_clk_ctrl(dsi_display->dsi_clk_handle,
...
{
	int rc = 1;
	int read_value = 0,read_value_slave =0;
	if(display == NULL)
		return rc;
	if (gpio_is_valid(display->panel->esd_config.esd_error_flag_gpio)&&gpio_is_valid(display->panel->esd_config.esd_error_flag_gpio_slave)) {
		...
		rc = gpio_request(display->panel->esd_config.esd_error_flag_gpio, "error-flag-gpio-slave");
		...
		pr_info("read_value=%d read_value_slave=%d\n", read_value, read_value_slave);
					...
					bool te_check_override)
{
	...
	if (status_mode == ESD_MODE_REG_READ) {
		...
		if ((panel->nt36523w_old_fpc)&&(rc < 0)&&!dsi_panel_need_rewrite_reg) {
			...
			if(dsi_panel_err_flag_continue_count == continue_esd_count){
				DSI_INFO("3 times continue error set dsi_panel_need_rewrite_reg = true\n");
				dsi_panel_need_rewrite_reg = true;
				#ifdef OPLUS_BUG_STABILITY
				count++;
				mm_fb_display_kevent_named(MM_FB_KEY_RATELIMIT_1H, "DisplayDriverID@@424$$ err flag continue 3 times. count = %d\n", count);
				#endif
			}
			...
			if(esd_occurred_count < record_count_occurr){
				...
				DSI_INFO("panel >10 store_index= %ld,%ld, value =%ld, %ld, result = %ld\n",
						...
						(esd_time_buffer[store_index] - esd_time_buffer[(store_index+1)%record_count_occurr]));
				if((esd_time_buffer[store_index] - esd_time_buffer[(store_index+1)%record_count_occurr]) < esd_time_region){
					dsi_panel_need_rewrite_reg = true;
					#ifdef OPLUS_BUG_STABILITY
					count++;
					DSI_INFO("panel rewrite ++++++ \n");
					mm_fb_display_kevent_named(MM_FB_KEY_RATELIMIT_1H, "DisplayDriverID@@425$$ 10 times ESD occurred in one hour. count = %d, ESD total - %d\n", count, esd_occurred_count);
					#endif
				}
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
	...
	if(strstr(saved_command_line, "panel_err=1") != NULL){
		DSI_ERR("judge for cmdline : dsi_panel_need_rewrite_reg = true\n");
		dsi_panel_need_rewrite_reg = true;
	}
#endif /* OPLUS_BUG_STABILITY */
...
#ifdef OPLUS_FEATURE_ADFR
		if (oplus_adfr_is_support()) {
			oplus_dsi_display_vsync_switch(display);
		}
#endif /*OPLUS_FEATURE_ADFR*/
...
int dsi_display_post_enable(struct dsi_display *display)
{
	...
	if(!strcmp(display->panel->name, "nt36523 lcd vid mode dsi panel")) {
		...
		notifier_data.id = 0;
		DSI_INFO("zsw nt36523 start resume touch\n");
	...
	if (!display->poms_pending) {
		...
		if (rc)
			DSI_ERR("[%s] failed to disable DSI panel, rc=%d\n",
				display->name, rc);
		oplus_post_on_backlight_status = 0x0;
#ifdef OPLUS_BUG_STABILITY
