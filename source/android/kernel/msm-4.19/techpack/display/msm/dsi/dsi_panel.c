...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (!tp_gesture_enable_flag() || g_shutdown_flag) {
	...
	if(!tp_gesture_enable_flag() || g_shutdown_flag) {
		...
		usleep_range(19000, 20000);  /* wait till spi sleep */
		if(gpio_is_valid(panel->reset_config.tp_cs_gpio)) {
...
#endif /*OPLUS_BUG_STABILITY*/
{
	...
	for (i = 0; i < count; i++) {
		...
		if (panel->oplus_priv.skip_mipi_last_cmd)
			cmds->msg.flags &= ~MIPI_DSI_MSG_LASTCOMMAND;
		if(panel->nt36523w_old_fpc && dsi_panel_need_rewrite_reg == true && type == DSI_CMD_SET_ON){
			if(i == (count - 2)){
	...
	u32 bl_lvl)
{
	...
	if (panel->is_hbm_enabled && (bl_lvl != 0)){
		pr_err("backlight is_hbm_enabled, skip!!\n");
		return 0;
	}

	if (oplus_display_get_hbm_mode()) {
		pr_err("backlight hbm mode, skip!!\n");
		return rc;
	}
	...
	if(OPLUS_DISPLAY_AOD_SCENE != get_oplus_display_scene()){
		...
		if (!strcmp(panel->name,"samsung S6E3HC3 dsc cmd mode panel")
			&& OPLUS_DISPLAY_NORMAL_SCENE == get_oplus_display_scene()) {
			...
			} else if ((enable_global_hbm_flags == 1)
				&& (bl_lvl < panel->bl_config.bl_normal_max_level)) {
			...
			if (rc)
		       ...
		       panel->name, rc);
		} else if(!strcmp(panel->oplus_priv.vendor_name, "AMB655XL08")) {
			if (bl_lvl > panel->bl_config.bl_normal_max_level) {
				enable_global_hbm_flags = 1;
				rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_ENTER_SWITCH);
				if (rc < 0)
		...
		#endif
			{
				if(0xE0 == payload[1])
					enable_global_hbm_flags = 1;
				if(0x20 == payload[1] && !strcmp(panel->oplus_priv.vendor_name, "AMB655XL08")) {
					if(oplus_display_panel_get_hbm_status()) {
						rc = ops->transfer(dsi->host, &msg);
						dsi_panel_tx_cmd_hbm_post_check(panel, DSI_CMD_HBM_OFF);
					} else if(enable_global_hbm_flags) {
						enable_global_hbm_flags = 0;
						rc = ops->transfer(dsi->host, &msg);
					}
...
static int dsi_panel_parse_misc_features(struct dsi_panel *panel)
{
	...
	panel->incell_screen = utils->read_bool(utils->data,
			...
			int topology_override)
{
	...
	dsi_for_each_child_node(timings_np, child_np) {
		...
		if (oplus_adfr_is_support()) {
			...
			oplus_dsi_panel_parse_panel_vsync_source(mode, utils);
		}
#endif
	}
	goto done;
