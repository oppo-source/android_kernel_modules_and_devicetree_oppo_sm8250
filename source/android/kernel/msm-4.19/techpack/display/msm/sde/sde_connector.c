...
static int _sde_connector_update_power_locked(struct sde_connector *c_conn)
{
	...
	if(c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		if(display_temp && display_temp->panel && display_temp->panel->nt36523w_ktz8866)
			SDE_INFO("zsw conn %d - dpms %d, lp %d, panel %d, last_panel_power_mode = %d\n", connector->base.id,
				c_conn->dpms_mode, c_conn->lp_mode, mode, c_conn->last_panel_power_mode);
	}
#endif /* OPLUS_BUG_STABILITY */
	...
	if (oplus_adfr_is_support()) {
		...
		if (prop_dirty) {
			...
			if (qsync_propval != c_conn->qsync_dynamic_min_fps) {
				SDE_DEBUG("updated qsync min fps %d -> %d\n",
				...
				struct drm_connector *connector)
{
	...
	while ((idx = msm_property_pop_dirty(&c_conn->property_info,
		...
		if(c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
			display_temp = (struct dsi_display *)c_conn->display;
			if(display_temp && display_temp->panel && display_temp->panel->nt36523w_ktz8866 && 7 != idx)
				SDE_INFO("zsw idx =%d\n", idx);
		}
#endif /* OPLUS_BUG_STABILITY */
		switch (idx) {
...
#ifdef OPLUS_FEATURE_ADFR
		if (oplus_adfr_is_support()) {
			params.qsync_dynamic_min_fps = c_conn->qsync_dynamic_min_fps;
		}
#endif
	}
