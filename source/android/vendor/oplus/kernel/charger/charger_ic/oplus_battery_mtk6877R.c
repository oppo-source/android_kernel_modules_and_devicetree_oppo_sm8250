...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (alarmtimer_get_rtcdev() && oplus_ccdetect_support_check() == false) {
		oplus_chip->alarm_clockid = ALARM_BOOTTIME;
		alarm_init(&oplus_chip->usbtemp_alarm_timer, oplus_chip->alarm_clockid, usbtemp_alarm_timer_func);
	} else {
