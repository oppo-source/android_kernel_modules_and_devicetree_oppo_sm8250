...
#define RA9530_WAIT_TIME             100             /* sec */
#define RA9530_SLEEP_TIME             50
...
#define PROC_0664 0664
#define CONFIG_NUM 0xFF
#define PAGE_256 256
...
static int g_count;
struct delayed_work ra9530_idt_timer_work;
static unsigned long long g_ble_timeout_cnt = NUM_0;
static unsigned long long g_verify_failed_cnt = NUM_0;
...
static int __ra9530_read_reg(struct oplus_ra9530_ic *chip, int reg, char *returnData, int count)
{
	...
	char cmd_buf[NUM_2]={ reg >> SHIFT_8, reg & CONFIG_NUM };
	int ret = NUM_0;
	int i;
	char val_buf[NUM_20] = {NUM_0};

	for (i = NUM_0; i < count; i++) {
		val_buf[i] = NUM_0;
	}
...
{
	int ret;
	unsigned char data[NUM_3] = { reg >> SHIFT_8, reg & CONFIG_NUM, val };

	ret = i2c_master_send(chip->client, data, NUM_3);
	if (ret < NUM_3) {
...
static int ra9530_write_reg_multi_byte(struct oplus_ra9530_ic *chip, int reg, char *cbuf, int length)
{
	...
	data_w[NUM_0] = reg >> SHIFT_8;
	data_w[NUM_1] = reg & CONFIG_NUM;

	memcpy(data_w + NUM_2, cbuf, length);
...
static void ra9530_check_clear_irq(struct oplus_ra9530_ic *chip)
{
	...
	rc = ra9530_read_reg(chip, RA9530_REG_INT_FLAG, chip->int_flag_data, NUM_4);
	if (rc) {
		chg_err("Couldn't read 0x%04x rc = %x\n", RA9530_REG_INT_FLAG, rc);
	} else {
		ra9530_write_reg_multi_byte(chip, RA9530_REG_INT_CLR, chip->int_flag_data, NUM_4);
		ra9530_config_interface(chip, RA9530_REG_RTX_CMD, HEX_2, CONFIG_NUM);
	}
}

static void check_int_enable(struct oplus_ra9530_ic *chip)
{
	char reg_int[NUM_3] = {NUM_0, NUM_0, NUM_0};
	int rc;
	...
	rc = ra9530_read_reg(chip, RA9530_REG_INT_EN, reg_int, NUM_3);
	if (rc) {
		chg_err("Couldn't read 0x%04x rc = %x\n", RA9530_REG_INT_EN, rc);
	} else {
		if ((reg_int[NUM_0] != HEX_DF) || (reg_int[NUM_1] != HEX_81)
			|| (reg_int[NUM_2] != HEX_2)) {
			reg_int[NUM_0] = HEX_DF;
			reg_int[NUM_1] = HEX_81;
			reg_int[NUM_2] = HEX_2;
			ra9530_write_reg_multi_byte(chip, RA9530_REG_INT_EN, reg_int, NUM_3);
		}
	}
}
...
void ra9530_check_point_function(struct work_struct *work)
{
	...
	oplus_chg_debug_info.wirelesspen_info.support = NUM_1;/* wireless pen check point */
...
static void ra9530_set_tx_mode(int value)
{
	...
	rc = ra9530_read_reg(chip, RA9530_REG_RTX_STATUS, &reg_tx, NUM_1);
	if (rc) {
		chg_err("Couldn't read 0x%04x rc = %x\n", RA9530_REG_RTX_STATUS, rc);
	} else {
		if (value && (RA9530_RTX_READY & reg_tx)) {
			chg_err("set tx enable\n");
			ra9530_config_interface(chip, RA9530_REG_RTX_CMD, HEX_1, CONFIG_NUM);
		} else if (!value && (RA9530_RTX_TRANSFER & reg_tx)) {
			chg_err("set tx disable\n");
			ra9530_config_interface(chip, RA9530_REG_RTX_CMD, HEX_4, CONFIG_NUM);
		} else {
...
{
	int rc = NUM_0;
	unsigned char addr[NUM_6];
	unsigned char decode_addr[NUM_6];
	unsigned long int ble_addr = NUM_0;

	rc = ra9530_read_reg(chip, RA9530_BLE_MAC_ADDR0, &addr[NUM_0], NUM_1);
	if (rc) {
		chg_err("Couldn't read 0x%02x rc = %x\n", RA9530_BLE_MAC_ADDR0, rc);
		return;
	}

	rc = ra9530_read_reg(chip, RA9530_BLE_MAC_ADDR1, &addr[NUM_1], NUM_1);
	if (rc) {
		chg_err("Couldn't read 0x%02x rc = %x\n", RA9530_BLE_MAC_ADDR1, rc);
		return;
	}

	rc = ra9530_read_reg(chip, RA9530_BLE_MAC_ADDR2, &addr[NUM_2], NUM_1);
	if (rc) {
		chg_err("Couldn't read 0x%02x rc = %x\n", RA9530_BLE_MAC_ADDR2, rc);
		return;
	}

	rc = ra9530_read_reg(chip, RA9530_BLE_MAC_ADDR3, &addr[NUM_3], NUM_1);
	if (rc) {
		chg_err("Couldn't read 0x%02x rc = %x\n", RA9530_BLE_MAC_ADDR3, rc);
		return;
	}

	rc = ra9530_read_reg(chip, RA9530_BLE_MAC_ADDR4, &addr[NUM_4], NUM_1);
	if (rc) {
		chg_err("Couldn't read 0x%02x rc = %x\n", RA9530_BLE_MAC_ADDR4, rc);
		return;
	}

	rc = ra9530_read_reg(chip, RA9530_BLE_MAC_ADDR5, &addr[NUM_5], NUM_1);
	if (rc) {
	...
	/* decode high 3 bytes */
	decode_addr[NUM_5] = ((addr[NUM_5] & DATA_0F) << NUM_4) | ((addr[NUM_2] & DATA_F0) >> NUM_4);
	decode_addr[NUM_4] = ((addr[NUM_4] & DATA_0F) << NUM_4) | ((addr[NUM_1] & DATA_F0) >> NUM_4);
	decode_addr[NUM_3] = ((addr[NUM_3] & DATA_0F) << NUM_4) | ((addr[NUM_0] & DATA_F0) >> NUM_4);

	/* decode low 3 bytes */
	decode_addr[NUM_2] = ((addr[NUM_2] & DATA_0F) << NUM_4) | ((addr[NUM_5] & DATA_F0) >> NUM_4);
	decode_addr[NUM_1] = ((addr[NUM_1] & DATA_0F) << NUM_4) | ((addr[NUM_4] & DATA_F0) >> NUM_4);
	decode_addr[NUM_0] = ((addr[NUM_0] & DATA_0F) << NUM_4) | ((addr[NUM_3] & DATA_F0) >> NUM_4);

	/* caculate final ble mac addr */
	ble_addr = decode_addr[NUM_5]  << SHIFT_16 | decode_addr[NUM_4] << SHIFT_8 | decode_addr[NUM_3];
	chip->ble_mac_addr = ble_addr << SHIFT_24 | decode_addr[NUM_2] << SHIFT_16 | decode_addr[NUM_1] << SHIFT_8 | decode_addr[NUM_0];
...
{
	int rc = NUM_0, i = NUM_0, count = NUM_3;
	unsigned char data[NUM_6];
	unsigned long int private_data = NUM_0;

	rc = ra9530_read_reg(chip, RA9530_PRIVATE_DATA_REG, &data[NUM_0], count);
	if (rc) {
	...
	for (i = NUM_0; i < count; i++) {
		chg_err("ra9530 private data %d = 0x%02x\n", i, data[count-NUM_1-i]);
		private_data += data[count-NUM_1-i];
		if (i < count - NUM_1) {
			private_data = private_data << SHIFT_8;
		}
	}
...
{
	char status_string[UEVENT_MESSAGE_MAX] = {NUM_0};
	char addr_string[UEVENT_MESSAGE_MAX] = {NUM_0};
...
static void ra9530_power_enable(struct oplus_ra9530_ic *chip, bool enable)
{
	...
	if (enable) {
		ra9530_set_vbat_en_val(chip, NUM_1);
		udelay(MS_1000);
		ra9530_set_booster_en_val(chip, NUM_1);
		chip->is_power_on = true;
	} else {
		ra9530_set_booster_en_val(chip, NUM_0);
		udelay(MS_1000);
		ra9530_set_vbat_en_val(chip, NUM_0);
...
static void ra9530_disable_tx_power(struct oplus_ra9530_ic *chip)
{
	...
	ra9530_set_tx_mode(NUM_0);
	ra9530_power_enable(chip, false);

	chip->ble_mac_addr = NUM_0;
	chip->private_pkg_data = NUM_0;
}

void ra9530_reg_print(void)
{
	char debug_data[NUM_6];

	ra9530_read_reg(ra9530_chip, RA9530_REG_RTX_ERR_STATUS, debug_data, NUM_2);
	chg_err("0x74 REG: 0x%02X 0x%02X\n",
			debug_data[NUM_0], debug_data[NUM_1]);

	ra9530_read_reg(ra9530_chip, RA9530_REG_RTX_STATUS, debug_data, NUM_1);
	chg_err("0x78 REG: 0x%02X\n", debug_data[NUM_0]);

	ra9530_read_reg(ra9530_chip, RA9530_BLE_MAC_ADDR0, &debug_data[NUM_0], NUM_1);
	ra9530_read_reg(ra9530_chip, RA9530_BLE_MAC_ADDR1, &debug_data[NUM_1], NUM_1);
	ra9530_read_reg(ra9530_chip, RA9530_BLE_MAC_ADDR2, &debug_data[NUM_2], NUM_1);
	ra9530_read_reg(ra9530_chip, RA9530_BLE_MAC_ADDR3, &debug_data[NUM_3], NUM_1);
	ra9530_read_reg(ra9530_chip, RA9530_BLE_MAC_ADDR4, &debug_data[NUM_4], NUM_1);
	ra9530_read_reg(ra9530_chip, RA9530_BLE_MAC_ADDR5, &debug_data[NUM_5], NUM_1);
	chg_err("0xbe-0xc3 REG: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n",
			debug_data[NUM_0], debug_data[NUM_1], debug_data[NUM_2],
			debug_data[NUM_3], debug_data[NUM_4], debug_data[NUM_5]);
}
...
static int RA9530_load_bootloader(struct oplus_ra9530_ic *chip)
{
	...
	rc = __ra9530_write_reg(chip, REG_3040, HEX_80); /* run M0*/
...
{
	unsigned char write_ack = NUM_0;
	int rc = NUM_0;

	rc = ra9530_write_reg_multi_byte(chip, REG_1008, fw_data,
							((codelength + NUM_8 + NUM_15) / NUM_16) * NUM_16);
	if (rc != NUM_0) {
		chg_err("<IDT UPDATE>ERROR: write multi byte data error!\n");
		goto LOAD_ERR;
	}

	rc = __ra9530_write_reg(chip, REG_1008, HEX_1);
	if (rc != NUM_0) {
	...
	} while ((write_ack & HEX_1) != NUM_0);

	/* check status*/
	if (write_ack != NUM_2) { /* not OK*/
		if (write_ack == NUM_4)
			chg_err("<IDT UPDATE>ERROR: WRITE ERR\n");
		else if (write_ack == NUM_8)
...
static int RA9530_Crc32_fw(struct oplus_ra9530_ic *chip, int start_addr, int fw_size, unsigned int crc)
{
	unsigned char write_ack = NUM_0;
	int rc = NUM_0;
	unsigned char crc_buff[NUM_4]={NUM_0, NUM_0, NUM_0, NUM_0};

	memcpy(crc_buff, (char *)&start_addr, NUM_2);
	rc = ra9530_write_reg_multi_byte(chip, REG_100A, crc_buff, NUM_2);

	memcpy(crc_buff, (char *)&fw_size, NUM_2);
	chg_err("RA9530_Crc32_fw fw_size buff[0]=%x buff[1]=%x] fw_size=0x%x!\n", crc_buff[0], crc_buff[1], fw_size);

	rc = ra9530_write_reg_multi_byte(chip, REG_100C, crc_buff, NUM_2);

	memcpy(crc_buff, (char *)&crc, NUM_4);
	chg_err("RA9530_Crc32_fw crc data buff[0]=%x buff[1]=%x] buff[2]=%x buff[3]=%x crc=%x!\n",
		crc_buff[NUM_0], crc_buff[NUM_1], crc_buff[NUM_2], crc_buff[NUM_3], crc);
	rc = ra9530_write_reg_multi_byte(chip, REG_1010, crc_buff, NUM_4);

	if (rc != NUM_0) {
		chg_err("<IDT UPDATE>ERROR: write multi byte data error!\n");
		goto LOAD_ERR;
	}
	rc = __ra9530_write_reg(chip, REG_1008, DATA_11);
	if (rc != NUM_0) {
		chg_err("<IDT UPDATE>ERROR: on OTP buffer validation\n");
		goto LOAD_ERR;
	}
...
static int RA9530_MTP(struct oplus_ra9530_ic *chip, unsigned char *fw_buf, int fw_size)
{
	...
	unsigned short int startaddr = NUM_0;
	unsigned short int checksum = NUM_0;
	unsigned short int codelength = NUM_0;
	int pure_fw_size = fw_size - RA9530_PURESIZE_OFFSET;
	unsigned int crc32value = NUM_0;

	memcpy(&crc32value, fw_buf + DATA_7EEC, NUM_4);
	...
	memset(fw_data, NUM_0, RA9530_FW_PAGE_SIZE);
	startaddr = pure_fw_size;
	checksum = startaddr;
	codelength = RA9530_FW_CODE_LENGTH;
	for (j = RA9530_FW_CODE_LENGTH - NUM_1; j >= NUM_0; j--)
		checksum += fw_data[j + NUM_8]; /* add the non zero values.*/

	checksum += codelength; /* finish calculation of the check sum*/
	memcpy(fw_data + NUM_2, (char *)&startaddr, NUM_2);
	memcpy(fw_data + NUM_4, (char *)&codelength, NUM_2);
	memcpy(fw_data + NUM_6, (char *)&checksum, NUM_2);
	...
	/* upgrade fw*/
	memset(fw_data, NUM_0, RA9530_FW_PAGE_SIZE);
	for (i = NUM_0; i < fw_size - NUM_20; i += RA9530_FW_CODE_LENGTH) {
		...
		memcpy(fw_data + NUM_8, fw_buf + i, RA9530_FW_CODE_LENGTH);

		j = fw_size - i;
		if (j < RA9530_FW_CODE_LENGTH) {
			j = ((j + NUM_15) / NUM_16) * NUM_16;
...
*/
	...
	startaddr = NUM_0;
	rc = RA9530_Crc32_fw(chip, startaddr, fw_size - NUM_20, crc32value);
	if (rc < NUM_0) {
		chg_err("crc32 err\n");
		goto MTP_ERROR;
	}
	...
	checksum = startaddr;
	codelength = RA9530_FW_CODE_LENGTH;
	memcpy(fw_data + NUM_8, fw_buf + startaddr, RA9530_FW_CODE_LENGTH);
	j = RA9530_FW_CODE_LENGTH - NUM_1;
	for (; j >= NUM_0; j--)
		checksum += fw_data[j + NUM_8]; /* add the non zero values.*/
...
{
	static int idt_update_retry_cnt = NUM_0;
	int rc = ERR_NUM, i = NUM_0, fw_version = NUM_0;
	char temp[NUM_5] = {NUM_0, NUM_0, NUM_0, NUM_0, NUM_0};
	unsigned char *fw_buf = NULL;
	int fw_size = NUM_0;
	int fw_ver_start_addr = NUM_0;
	...
	if (rc) {
		...
		chg_err("<IDT UPDATE>The new fw version: %02x %02x %02x %02x beta version:%02x\n",
				fw_buf[fw_ver_start_addr + HEX_5], fw_buf[fw_ver_start_addr + HEX_6],
				fw_buf[fw_ver_start_addr + HEX_7], fw_buf[fw_ver_start_addr + HEX_8], fw_buf[fw_ver_start_addr]);

		if ((temp[NUM_0] != fw_buf[fw_ver_start_addr + HEX_5])\
			...
			if (RA9530_MTP(chip, fw_buf, fw_size) == NUM_0) {
				chg_err("<IDT UPDATE>Update success!!!\n");
			} else {
				idt_update_retry_cnt++;
				if (idt_update_retry_cnt > NUM_5) {
...
{
	unsigned short recv_data = NUM_0;
	int rc = NUM_0, count_limit = NUM_0;
	static int count = NUM_0;

	rc = ra9530_read_reg(chip, RA9530_REG_RTX_ERR_STATUS, (char *)&recv_data, NUM_2);
	if (rc) {
...
{
	unsigned long int addr = NUM_0;
	unsigned long int pdata = NUM_0;
	...
	if ((((addr >> SHIFT_40) & CONFIG_NUM) ^ ((addr >> SHIFT_16) & CONFIG_NUM)) != ((pdata >> SHIFT_16) & CONFIG_NUM)) {
		return false;
	}
	if ((((addr >> NUM_32) & CONFIG_NUM) ^ ((addr >> SHIFT_8) & CONFIG_NUM)) != ((pdata >> SHIFT_8) & CONFIG_NUM)) {
		return false;
	}
	if ((((addr >> SHIFT_24) & CONFIG_NUM) ^ (addr & CONFIG_NUM)) != (pdata & CONFIG_NUM)) {
		return false;
	}

	chg_err("ra9530_valid_check: %02x %02x %02x.\n", ((pdata >> SHIFT_16) & CONFIG_NUM), ((pdata >> SHIFT_8) & CONFIG_NUM), (pdata & CONFIG_NUM));
	return true;
}

static void ra9530_check_private_flag(struct oplus_ra9530_ic *chip)
{
	uint8_t rc = NUM_0, val = NUM_0;

	rc = ra9530_read_reg(chip, RA9530_REG_CHARGE_PERCENT, &val, NUM_1);
	if (rc) {
	...
	ra9530_disable_tx_power(chip);
	if (NUM_100 == val) {               /* charger full, can stop charging now */
...
/* for ato cal ping time end*/
	...
	if (chip->int_flag_data[NUM_1] & RA9530_INT_FLAG_BLE_ADDR) {
	...
	if (chip->int_flag_data[NUM_2] & RA9530_INT_FLAG_PRIVATE_PKG) {
		if (!chip->private_pkg_data) {
			chg_err("GET_PRIVATE_PKG.\n");
			chip->present = NUM_1;
			notify_pen_state(NUM_1);
			check_int_enable(chip);
			ra9530_set_private_data(chip);
		}
	}

	if (chip->int_flag_data[NUM_1] & RA9530_INT_FLAG_STOP_CHARGING) {
		chg_err("GET_PRIVATE_FLAG!!!\n");
		ra9530_check_private_flag(chip);
	}

	if (chip->int_flag_data[NUM_0] & RA9530_INT_FLAG_EPT) {
		ra9530_ept_type_detect_func(chip);
	}

	if (chip->int_flag_data[NUM_0] & RA9530_INT_FLAG_DPING) {
		/*DPING int*/
		chg_err("DPING int!!!\n");
	}

	if (chip->int_flag_data[NUM_0] & RA9530_INT_FLAG_SS) {
		/*SS int*/
		chip->present = NUM_1;
		notify_pen_state(NUM_1);
		check_int_enable(chip);
	...
	if (chip->is_power_on) {                /* avoid some i2c error when powered off before check here */
		...
		if (ra9530_get_idt_int_val() == NUM_0) {
...
static int ra9530_vbat_en_gpio_init(struct oplus_ra9530_ic *chip)
{
	...
	gpio_direction_output(chip->vbat_en_gpio, NUM_0);
...
void ra9530_set_vbat_en_val(struct oplus_ra9530_ic *chip, int value)
{
	...
	if (value) {
		gpio_direction_output(chip->vbat_en_gpio, NUM_1);
		pinctrl_select_state(chip->pinctrl,
				chip->vbat_en_default);
	} else {
		gpio_direction_output(chip->vbat_en_gpio, NUM_0);
...
static int ra9530_booster_en_gpio_init(struct oplus_ra9530_ic *chip)
{
	...
	gpio_direction_output(chip->booster_en_gpio, NUM_0);
	pinctrl_select_state(chip->pinctrl,
			chip->booster_en_sleep);
...
void ra9530_set_booster_en_val(struct oplus_ra9530_ic *chip, int value)
{
	...
	if (value) {
		gpio_direction_output(chip->booster_en_gpio, NUM_1);
		pinctrl_select_state(chip->pinctrl,
				chip->booster_en_active);
	} else {
		gpio_direction_output(chip->booster_en_gpio, NUM_0);
...
{
	int rc = NUM_0;
	unsigned char temp[NUM_2] = {NUM_0, NUM_0};

	if (!chip) {
		return;
	}

	rc = ra9530_read_reg(chip, RA9530_REG_CURRENT_MA, temp, NUM_2);
	if (rc) {
		chg_err("Couldn't read tx_current reg 0x006E rc = %d\n", rc);
	}
	chip->tx_current = temp[NUM_1] << SHIFT_8;
	chip->tx_current += temp[NUM_0];
	chg_err("tx_current: %02x %02x  dec:%d mA\n", temp[NUM_0], temp[NUM_1], chip->tx_current);

	rc = ra9530_read_reg(chip, RA9530_REG_AVR_CURRENT_MA, temp, NUM_2);
	if (rc) {
		chg_err("Couldn't read tx_current reg 0x006E rc = %d\n", rc);
	}
	chip->tx_avr_current = temp[NUM_1] << SHIFT_8;
	chip->tx_avr_current += temp[NUM_0];
	chg_err("tx_avr_current: %02x %02x  dec:%d mA\n", temp[NUM_0], temp[NUM_1], chip->tx_avr_current);


	rc = ra9530_read_reg(chip, RA9530_REG_VOLTAGE_MV, temp, NUM_2);
	if (rc) {
		chg_err("Couldn't read tx_voltage reg 0x0070 rc = %d\n", rc);
	}
	chip->tx_voltage = temp[NUM_1] << SHIFT_8;
	chip->tx_voltage += temp[NUM_0];
	chg_err("tx_voltage: %02x %02x  dec:%d mV\n", temp[NUM_0], temp[NUM_1], chip->tx_voltage);

	rc = ra9530_read_reg(chip, RA9530_REG_OUTPUT_VOLTAGE_MV, temp, NUM_2);
	if (rc) {
		chg_err("Couldn't read tx_output_voltage reg 0x0082 rc = %d\n", rc);
	}
	chip->tx_output_voltage = temp[NUM_1] << SHIFT_8;
	chip->tx_output_voltage += temp[NUM_0];
	chg_err("tx_output_voltage: %02x %02x  dec:%d mV\n", temp[NUM_0], temp[NUM_1], chip->tx_output_voltage);
...
void ra9530_timer_inhall_function(struct work_struct *work)
{
	...
	if (g_pen_ornot == NUM_1) {/* pen ,read voltage current return */
		...
		__pm_relax(chip->bus_wakelock);
		return;
	} else if (g_pen_ornot == NUM_0) {/* not pen continue check */
		g_count++;
	}

	if (g_pen_ornot == NUM_0 && g_count >= RA9530_WAIT_TIME) {/* no ss int for RA9530_WAIT_TIME sec, power off */
...
static void ra9530_do_switch(struct oplus_ra9530_ic *chip, uint8_t status)
{
	...
	if (status == PEN_STATUS_NEAR) {/* hall near, power on*/
		...
		msleep(MS_100);
		ra9530_set_protect_parameter(chip);
		ra9530_set_tx_mode(MODE_1);
		chip->power_enable_reason = PEN_REASON_NEAR;
		chip->tx_start_time = ktime_to_ms(ktime_get());
		schedule_delayed_work(&ra9530_idt_timer_work, round_jiffies_relative(msecs_to_jiffies(MS_50)));
...
static void ra9530_power_onoff_switch(int value)
{
	...
	__pm_stay_awake(chip->bus_wakelock);
	wait_event_interruptible_timeout(i2c_waiter, chip->i2c_ready, msecs_to_jiffies(MS_50));
	mutex_lock(&chip->flow_mutex);

	chip->pen_status = value > NUM_0 ? PEN_STATUS_NEAR : PEN_STATUS_FAR;
	if (chip->i2c_ready) {
		ra9530_do_switch(chip, chip->pen_status);
	} else {
...
static int ra9530_idt_gpio_init(struct oplus_ra9530_ic *chip)
{
	...
	chip->vbat_en_gpio = of_get_named_gpio(node, "qcom,vbat_en-gpio", NUM_0);
	if (chip->vbat_en_gpio < NUM_0) {
	...
	chip->booster_en_gpio = of_get_named_gpio(node, "qcom,booster_en-gpio", NUM_0);
	if (chip->booster_en_gpio < NUM_0) {
...
static ssize_t ra9530_reg_store(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	write_data[len] = '\0';
	if (write_data[len - NUM_1] == '\n') {
		write_data[len - NUM_1] = '\0';
	}

	ra9530_add = (int)simple_strtoul(write_data, NULL, NUM_0);

	chg_err("%s:received data=%s, ra9530_register address: 0x%02x\n", __func__, write_data, ra9530_add);

	rc = ra9530_read_reg(ra9530_chip, ra9530_add, &val_buf, NUM_1);
	if (rc) {
...
static void init_ra9530_add_log(void)
{
	...
	p = proc_create("ra9530_add_log", PROC_0664, NULL, &ra9530_add_log_proc_fops);
	if (!p) {
...
static ssize_t ra9530_data_log_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	critical_log = (int)simple_strtoul(write_data, NULL, NUM_0);
	if (critical_log > CONFIG_NUM) {
		critical_log = CONFIG_NUM;
	}

	chg_err("%s:received data=%s, ra9530_data=%x\n", __func__, write_data, critical_log);

	rc = ra9530_config_interface(ra9530_chip, ra9530_add, critical_log, CONFIG_NUM);
	if (rc) {
...
static void init_ra9530_data_log(void)
{
	...
	p = proc_create("ra9530_data_log", PROC_0664, NULL, &ra9530_data_log_proc_fops);
	if (!p)
...
static ssize_t fw_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_ra9530_ic *chip = NULL;
	int rc;
	char temp[NUM_4] = {NUM_0, NUM_0, NUM_0, NUM_0};
	...
	chip->idt_fw_version = (temp[NUM_3] << SHIFT_24) | (temp[NUM_2] << SHIFT_16) | (temp[NUM_1] << SHIFT_8) | temp[NUM_0];
...
{
	int i = NUM_0, count = NUM_0;
	char tmp_buf[DEBUG_BUFF_SIZE] = {NUM_0};
	struct oplus_ra9530_ic *chip = NULL;
	...
	for (i = NUM_0; i < PEN_OFF_REASON_MAX; i++) {
...
static int init_wireless_device(struct oplus_ra9530_ic *chip)
{
	...
	status = alloc_chrdev_region(&devt, NUM_0, NUM_1, "tx_wireless");
...
static int ra9530_identity_check(struct oplus_ra9530_ic *chip)
{
	...
	ra9530_power_enable(chip, true);
	msleep(MS_10);
	rc = ra9530_read_reg(chip, RA9530_REG_IDENTITY, buf, NUM_5);
	...
	chg_err("chip id = 0x%02x%02x\n", buf[NUM_0], buf[NUM_4]);
	 if ((buf[NUM_0] == RT_ID_H) && (buf[NUM_4] == RT_ID_L)) {
		rc = NUM_0;
	} else {
		rc = ERR_NUM;
	}
...
static int ra9530_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&ra9530_idt_timer_work, ra9530_timer_inhall_function);
...
int ra9530_driver_init(void)
{
	...
	if (i2c_add_driver(&ra9530_i2c_driver) != NUM_0) {
