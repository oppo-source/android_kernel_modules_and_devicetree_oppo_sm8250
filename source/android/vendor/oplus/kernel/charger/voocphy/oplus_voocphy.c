...
#define CPUFREQ_CHECK_TIME 60000
...
void voocphy_cpufreq_init(void)
{
	...
	cancel_delayed_work_sync(&g_voocphy_chip->cpufreq_check_work);
	schedule_delayed_work(&g_voocphy_chip->cpufreq_check_work, msecs_to_jiffies(CPUFREQ_CHECK_TIME));
#endif
}
...
void voocphy_cpufreq_check_work(struct work_struct *work)
{
	if (!g_voocphy_chip)
		return;
	cpuboost_charge_event(CPU_CHG_FREQ_STAT_AUTO);
...
bool oplus_voocphy_check_fastchg_real_allow(void)
{
	...
	//batt_soc /= 100;
	batt_temp = oplus_chg_match_temp_for_chging();
	if (chip->batt_fake_temp) {
		batt_temp = chip->batt_fake_temp;
	}
	if (chip->vooc_normal_high_temp != -EINVAL
...
void oplus_voocphy_init(struct oplus_voocphy_manager *chip)
{
	...
	INIT_DELAYED_WORK(&chip->cpufreq_check_work, voocphy_cpufreq_check_work);
	g_voocphy_chip = chip;
}
