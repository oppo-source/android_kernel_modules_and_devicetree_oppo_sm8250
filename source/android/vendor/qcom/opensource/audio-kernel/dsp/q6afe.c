...
static int get_smartpa_pm_result_apr(void *buf, int cmd_size)
{
	...
	pr_info("%s: port:%d, topo_id:%d\n", __func__, port, topo_id);
...
int send_tfa_cal_apr(void *buf, int cmd_size, bool bRead)
{
	...
	#ifdef OPLUS_FEATURE_TFA98XX_VI_FEEDBACK
	if (1 == get_smartpa_tdm_port_id_flag()) {
		port_id = afe_port_id_tfa_rx;
	}

	pr_debug("%s:  port_id = %d  line = %d\n",__func__, port_id,__LINE__);
	#endif /*OPLUS_FEATURE_TFA98XX_VI_FEEDBACK*/
...
//Liang.Huang@MM.AUDIO.DRIVER.CODEC tfa98xx Speaker protection algorithm upgrade
