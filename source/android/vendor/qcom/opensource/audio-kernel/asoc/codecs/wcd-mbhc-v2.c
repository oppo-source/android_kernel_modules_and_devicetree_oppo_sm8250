...
static void wcd_check_plug_irq_fn(struct work_struct *work)
{
	...
	if (mbhc && mbhc->mbhc_cfg && mbhc->mbhc_cfg->enable_usbc_analog) {
		...
		pr_info("%s: mode = %lu, hph_status=%d\n", __func__, mbhc->typec_mode, mbhc->hph_status);

		if (((mbhc->typec_mode == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER) && (mbhc->hph_status == 0)) || \
				((mbhc->typec_mode == POWER_SUPPLY_TYPEC_NONE) && (mbhc->hph_status !=0))) {
					   ...
					   unsigned long mode, void *ptr)
{
	...
	#else /* OPLUS_ARCH_EXTENDS */
	dev_info(mbhc->component->dev, "%s: mode = %lu, hph_status=%d\n", __func__, mode, mbhc->hph_status);
	#endif /* OPLUS_ARCH_EXTENDS */

	#ifdef OPLUS_FEATURE_MM_FEEDBACK
	if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->enable_usbc_analog && mbhc->hp_wake_lock) {
		cancel_delayed_work_sync(&mbhc->hp_irq_chk_work);
		mbhc->typec_mode = mode;
