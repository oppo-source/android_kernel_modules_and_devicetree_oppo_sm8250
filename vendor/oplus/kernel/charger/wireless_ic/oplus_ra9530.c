...
static void ra9530_set_tx_mode(int value)
{
	...
	char reg_tx;
	int rc;
	unsigned char Wbuff[NUM_2] = {0xA0, 0x0F};
	...
	if (rc) {
		...
		if (value && (RA9530_RTX_READY & reg_tx)) {
			...
			ra9530_write_reg_multi_byte(chip, RA9530_REG_UVLO_THD, Wbuff, NUM_2);
...
static int RA9530_load_fw(struct oplus_ra9530_ic *chip, unsigned char *fw_data, int codelength)
{
	...
	if (write_ack != NUM_2) { /* not OK*/
		if (write_ack == HEX_1) {
			chg_err("<IDT UPDATE>ERROR:BUSY,EEPROM Write is in progress\n");
		} else if (write_ack == HEX_14) {
			chg_err("<IDT UPDATE>ERROR:Check Sum Error\n");
		} else if (write_ack == HEX_24) {
			chg_err("<IDT UPDATE>ERROR:Comparison Failure\n");
		} else if (write_ack == HEX_34) {
			chg_err("<IDT UPDATE>ERROR:Address not aligned\n");
		} else if (write_ack == HEX_44) {
			chg_err("<IDT UPDATE>ERROR:Length not supported\n");
		} else {
			chg_err("<IDT UPDATE>ERROR: UNKNOWN ERR(0x%02x)\n", write_ack);
		}
...
static int RA9530_MTP(struct oplus_ra9530_ic *chip, unsigned char *fw_buf, int fw_size)
{
	...
	if (rc != NUM_0) {
		chg_err("<IDT UPDATE>load bootloader failed!\n");
		return rc;
	}
...
static int ra9530_check_idt_fw_update(struct oplus_ra9530_ic *chip, bool force_update)
{
	...
	int rc = ERR_NUM, mtp_result = NUM_0;
	char temp[NUM_5] = {NUM_0, NUM_0, NUM_0, NUM_0, NUM_0};
